...
public class AuthDemo {
    ...
    public static void main(String[] args){
        ...
        try {
            ...
            com.github.ontio.account.Account payerAcct = ontSdk.getWalletMgr().getAccount(payer.address,password,payer.salt.getBytes());
...
//                String txhash = ontSdk.nativevm().auth().sendTransfer(identity.ontid,password,contractAddr,dids.get(0).ontid,1,account,ontSdk.DEFAULT_GAS_LIMIT,0);
                String txhash = ontSdk.nativevm().auth().assignFuncsToRole(dids.get(0).ontid,password,dids.get(0).controls.get(0).salt.getBytes(),contractAddr,"role",new String[]{"foo"},1,account,ontSdk.DEFAULT_GAS_LIMIT,0);
