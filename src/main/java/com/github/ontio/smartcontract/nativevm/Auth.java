...
//        Transaction tx = sdk.vm().makeInvokeCodeTransaction(contractAddr,"initContractAdmin",null, payerAcct.getAddressU160().toBase58(),gaslimit,gasprice);
        ...
        list.add(new Struct().add(adminOntId.getBytes()));
        byte[] arg = NativeBuildParams.createCodeParamsScript(list);

        Transaction tx = sdk.vm().buildNativeParams(new Address(Helper.hexToBytes(contractAddress)),"initContractAdmin",arg,payerAcct.getAddressU160().toBase58(),gaslimit,gasprice);
